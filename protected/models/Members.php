<?php

/**
 * This is the model class for table "members".
 *
 * The followings are the available columns in table 'members':
 * @property string $id
 * @property string $code
 * @property string $name
 * @property string $mail
 * @property integer $mobile
 * @property string $phone
 * @property string $company
 * @property integer $callStatus
 * @property string $metaData
 * @property integer $modifier
 * @property string $modify
 * @property integer $status
 * @property integer $takeTime
 * @property string $uniqueId
 */
class Members extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return Members the static model class
	 */
	public static function model($className=__CLASS__)
	{
		return parent::model($className);
	}

	/**
	 * @return string the associated database table name
	 */
	public function tableName()
	{
		return 'members';
	}

	/**
	 * @return array validation rules for model attributes.
	 */
	public function rules()
	{
		// NOTE: you should only define rules for those attributes that
		// will receive user inputs.
		return array(
			array('mobile, callStatus, modifier, status, takeTime', 'numerical', 'integerOnly'=>true),
			array('code, uniqueId', 'length', 'max'=>20),
			array('name, mail, phone', 'length', 'max'=>60),
			array('company', 'length', 'max'=>120),
			array('modify', 'length', 'max'=>210),
			array('metaData', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, code, name, mail, mobile, phone, company, callStatus, metaData, modifier, modify, status, takeTime, uniqueId', 'safe', 'on'=>'search'),
		);
	}

	/**
	 * @return array relational rules.
	 */
	public function relations()
	{
		// NOTE: you may need to adjust the relation name and the related
		// class name for the relations automatically generated below.
		return array(
		);
	}

	/**
	 * @return array customized attribute labels (name=>label)
	 */
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'code' => 'Code',
			'name' => 'Name',
			'mail' => 'Mail',
			'mobile' => 'Mobile',
			'phone' => 'Phone',
			'company' => 'Company',
			'callStatus' => 'Call Status',
			'metaData' => 'Meta Data',
			'modifier' => 'Modifier',
			'modify' => 'Modify',
			'status' => 'Status',
			'takeTime' => 'Take Time',
			'uniqueId' => 'Unique',
		);
	}

	/**
	 * Retrieves a list of models based on the current search/filter conditions.
	 * @return CActiveDataProvider the data provider that can return the models based on the search/filter conditions.
	 */
	public function search()
	{
		// Warning: Please modify the following code to remove attributes that
		// should not be searched.

		$criteria=new CDbCriteria;

		$criteria->compare('id',$this->id,true);
		$criteria->compare('code',$this->code,true);
		$criteria->compare('name',$this->name,true);
		$criteria->compare('mail',$this->mail,true);
		$criteria->compare('mobile',$this->mobile);
		$criteria->compare('phone',$this->phone,true);
		$criteria->compare('company',$this->company,true);
		$criteria->compare('callStatus',$this->callStatus);
		$criteria->compare('metaData',$this->metaData,true);
		$criteria->compare('modifier',$this->modifier);
		$criteria->compare('modify',$this->modify,true);
		$criteria->compare('status',$this->status);
		$criteria->compare('takeTime',$this->takeTime);
		$criteria->compare('uniqueId',$this->uniqueId,true);

		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
		));
	}
}